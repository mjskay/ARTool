...
author: "Matthew Kay, Lisa A. Elkin"
date: "`r Sys.Date()`"
...
Contrasts involving levels of **any number of factors** can be conducted using the ART-C method.

This page explains when and why a separate aligning-and-ranking procedure is needed to conduct contrasts and demonstrates how to conduct contrasts within the ART paradigm.
...
3. [Tests of differences in pairwise combinations of levels between factors in interactions](#tests-of-differences-in-pairwise-combinations-of-levels-between-factors-in-interactions):
   Interaction contrasts must be conducted with ART-C as they **are not valid** in ART
...
**Within a single factor** ART and ART-C are mathematically equivalent, so we would expect the contrast tests for ART and ART-C to have the same results, and they do.
...
The ART-C procedure however, was developed to align and rank data specifically for contrasts involving levels from any number of factors.

```{r}
art.con(m.art, "X1:X2")
```

Like the linear model, art.con  correctly estimates the difference between `A,C - A,D` as `~0`. In fact, its results agree with the linear model for all contrasts conducted. (Note that the art.con and linear model results appear in a different order; specifically A C - A D appears second for the linear model and first for art.con)

The syntax used above is consistent with *term* syntax used by artlm. art.con also accepts the *formula* syntax accepted by emmeans. We can conduct the same contrasts as above using the following syntax:
```{r}
art.con(m.art, ~X1*X2)
```
...
Before ART-C was created and added to this package, the only way to conduct multifactor contrasts within the ART paradigm was to test _differences of differences_; e.g., for the interaction `X1:X2`, we might ask, is the difference `A - B` different when `X2 = C` compared to when `X2 = D`?. We can still test this using the `interaction` argument to art.con. Note that when the `interaction` argument is supplied to art.con, differences of differences are tested on data that has been aligned-and-ranked using the **original** ART method (i.e., the data is **not** aligned-and-ranked using the ART-C method).
